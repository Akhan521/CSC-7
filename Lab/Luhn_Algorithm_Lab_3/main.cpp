 ...
 * File:   main.cpp
 * Author: Aamir Khan
 * Created on March 16, 2022, 9:30 AM
 * Purpose: Luhn Algorithm Lab 3
 */
...
void trnspse(char []);    //Randomly transposes 2 side-by-side digits.
void flip2Dig(char []);   //Randomly flips 2 digits of our CC number.
...
int main(int argc, char** argv) {
    ...
    char option;         //To choose 1 of 2 options.
    
    //Choosing whether to transpose or flip 2 digits.
    cout<<"Would you like to:\n"
        <<"1. Transpose 2 digits of a CC and see CC statistics?\n"
        <<"2. Flip 2 digits of a CC and see CC statistics?\n";
    cin>>option;
    switch(option){
        case '1':{
            ...
            //Transpose 2 digits.
            cout<<"CC Before 2 Transposed Digits = "<<crdCard<<endl;
            trnspse(crdCard);
            cout<<"CC After 2 Transposed Digits  = "<<crdCard<<endl;
        ...
        case '2':{
            ...
            char *crdCard=genCC(ALL);
            //Transpose 2 digits.
            cout<<"CC Before 2 Flipped Digits = "<<crdCard<<endl;
            flip2Dig(crdCard);
            cout<<"CC After 2 Flipped Digits  = "<<crdCard<<endl;
            //Test the modified CC # for validity.
            validCC(crdCard)?nVld++:nInvld++;
            //Deallocate memory for every CC we create using genCC(TYPE).
            delete []crdCard;
...
void flip2Dig(char cc[]){
    ...
    cc[rand()%(numDgts-1)]=rndDgit(); //1st digit flipped.
    cc[rand()%(numDgts-1)]=rndDgit(); //2nd digit flipped.
}

void trnspse(char cc[]){
    char temp;      //A temporary variable used for swapping.
    int idx1,idx2;  //The indices of the 2 elements being swapped.
    //To transpose, we need 2 side-by-side indices.
    //Depending on what card we have, the range of our indices to choose from is different.
    //For ex: If we had VISA, we'd be able to choose anything between 0-13.
    //We stop at 13 because if our index is 13, we swap w/ the value @ index 14.
    //We don't want to swap index 14 w/ 15 since 15 has the Luhn digit, so we stop before 14.
    //This is why we use rand()%(numDgts-2) which returns 0-13 for VISA.
    int numDgts;            //The # of digits our CC has.
    cc[0]=='3'?numDgts=15:  //AMEX has 15 digits.
               numDgts=16;  //The rest have 16 digits.
    //We choose a random index for the 1st element to be swapped.
    idx1=rand()%(numDgts-2); 
    //We choose the next index for the 2nd element to be swapped.
    //Index 1 and 2 need to be side-by-side for us to transpose.
    idx2=idx1+1;
    //Perform the swap now that we have our indices for the elements being swapped:
    temp=cc[idx1];
    cc[idx1]=cc[idx2];
    cc[idx2]=temp;
}
